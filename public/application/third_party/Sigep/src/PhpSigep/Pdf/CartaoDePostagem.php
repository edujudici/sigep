    ...
    private function _render($dest='', $filename='')
    {
        ...
        while (count($objetosPostais)) {
            ...
            for ($area = 0; $area < 1; $area++) {
                ...
                //  "Volume: $index/$total    ".
                $this->t($this->pdf->w, 'Peso(kg): ' . ((float)$objetoPostal->getPeso()) . $nf , 1, 'C',  null);
                ...
                $this->t(0, 'Nome Legível:_____________________________________________________',1, 'L',  null);
                $this->pdf->SetXY(1, $this->pdf->GetY() + 1);
                $this->t(0, 'Documento:_______________________________________________________',1, 'L',  null);
                 $this->pdf->SetXY(0, 3);
                // Destinatário 
                ...
                if ($destino instanceof \PhpSigep\Model\DestinoNacional) {
                    ...
                    $this->setFillColor(222, 222, 222);
                    $this->pdf->gdImage($semaCodeGD, 40, 3, 25);//alterar para caber a logo
                ...
                if(!empty($numeroPedido)) {
                    ...
                    $this->pdf->SetFont('', 'B');
                    $this->t(0, 'Pedido: '.$numeroPedido, 0, 'R', 63);
                }
                    // Etiqueta do CEP
                    $hCepBarCode = 22;
                    $wCepBarCode = 47;
                ...
                $this->pdf->SetXY(1, $this->pdf->GetY() + 3);
                $this->t(0, '',10, 'L',  10);
