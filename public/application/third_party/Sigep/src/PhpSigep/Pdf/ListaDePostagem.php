...
class ListaDePostagem
{
    ...
    public function render($dest='', $fileName = '')
    {
        ...
        if ($pdfContent = Bootstrap::getConfig()->getCacheInstance()->getItem($cacheKey)) {
            ...
            $this->writeHeader_1($pdf, $k, $wInner);
            $this->writeHeader($pdf, $k, $wInner);
            $pdf->Ln();
            $this->writeList($pdf, $k, $wInner);
            $pdf->Ln();
            $pdf->Ln();
            //$this->writeBottom();
            //$this->writeFooter();
    ...
    private function writeHeader_1($pdf, $k, $wInner)
    {

        $yHeaderRect = $pdf->y + 15 / $k;
        $pdf->SetY($yHeaderRect);
        $pdf->SetFont('', 'B', 13);
        $pdf->CellXp($wInner, ' PRÉ - LISTA DE POSTAGEM - PLP - SIGEP WEB', 'C', 1);
        
         $pdf->Rect(0.388, 0.8, 7.488, 0.5);
...
// Cria o cabeçalho
        ...
        $pdf->SetY($yHeaderRect);
        $pdf->SetFont('', 'B', 10);
        $pdf->CellXp($wInner, 'SIGEP WEB - Gerenciador de Postagens dos Correios', 'L', 1);
        ...
        $this->labeledText($pdf, 'Contrato: ', $plp->getAccessData()->getNumeroContrato(), $wHeaderCols, 1);
        $this->labeledText($pdf, 'Cliente: ', $remetente->getNome(), $wHeaderCols, 1);
        $this->labeledText($pdf, 'Telefone de contato: ', $remetente->getTelefone(), $wHeaderCols, 1);
        $this->labeledText($pdf, 'Email de contato: ', "", $wHeaderCols, 1);

        $yAboveFone = $pdf->y;
        $pdf->y -= ($pdf->getLineHeigth() * 3);
        $pdf->x += $wHeaderCols;
        $yAboveAddress = $pdf->y;

        $pdf->CellXp($wInner, 'Nº PLP: '.$this->idPlpCorreios, 'C', 0, 0);
    ...
    private function writeList($pdf, $k, $wInner)
    {

// Cria o cabeçalho
        $yHeaderRect = $pdf->y + 45 / $k;
        $pdf->SetY($yHeaderRect);
        ...
        $pdf->setLineHeightPadding(50 / $k);
        $this->labeledText($pdf, 'Quantidade:', '', $wHeaderCols, 1);
